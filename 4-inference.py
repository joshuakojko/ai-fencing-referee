    ...
    if prediction != 2:  # If there's a clear priority
        if prediction == 0:  # Left priority - multiple arrows pointing left
            for i in range(3):  # Draw 3 arrows
                ...
                cv2.arrowedLine(
                    frame, start_point, end_point, color, arrow_thickness, tipLength=0.5
                )
...
def analyze_video(
    video_path, model_path, labels_dir, output_dir, window_size=15, stride=5
):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                cell_features.extend(
                    [motion_intensity, np.cos(direction), np.sin(direction)]
                )
    ...
    if len(feature_buffer) > 0:
        ...
        final_output_path = (
            output_dir / category / ground_truth / f"{clip_id}_analyzed.mp4"
        )
    ...
    for video_path in clips_dir.glob("*.mp4"):
        ...
        try:
            ...
            if output_path:
                ...
                category = (
                    "correct_predictions"
                    if "correct_predictions" in str(output_path)
                    else "incorrect_predictions"
                )
                ground_truth = get_ground_truth(video_path, labels_dir)
